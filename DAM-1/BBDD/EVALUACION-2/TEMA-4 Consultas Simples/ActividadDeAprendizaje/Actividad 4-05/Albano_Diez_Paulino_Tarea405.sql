...
select * from votos where fecha=20180107;
...
select distinct usuario from votos where fecha between 20180102 and 20180106 order by usuario asc;

/*3.- Obtén toda la información sobre los votos registrados en enero de 2018 ordenando 
por número de canción.*/

select * from votos where fecha between 20180101 and 20180131 order by cancion;
...
sintaxis de year es:
year(fecha)*/
select codgrupo,nombre,fechaEstreno from grupos where year(fechaEstreno)<2010 order by fechaEstreno;

/*5.- Obtén los datos de todas las canciones del concurso que tienen una duración 
comprendida entre 3 y 4 minutos.*/
select * from canciones where duracion between 000300 and 000400;
...
select * from votos where fecha>=curdate() order by fecha;
